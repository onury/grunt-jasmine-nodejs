...
- **reporters.terminal**
    An alternative reporter that outputs the detailed test results to the console, with colors. This terminal displays the test results as they are ran, as opposed to the console reporter, which shows a dot for each test, and then (optionally) shows the results once the test run has completed.

    + **color** — Type: `Boolean` Default: `false`
    Specifies whether the output should have colored text.

    + **showStack** — Type: `Boolean` Default: `false`
    Specifies whether to show stack traces for failed specs.

    + **verbosity** — Type: `Number` Default: `2`
    Specifies how verbose the output should be. Ranges from 0 (sparse) to 3 (very verbose).
...
- **v1.1.0** (2015-04-20)
    + Added terminal reporter. (Fixes [Issue #7](https://github.com/onury/grunt-jasmine-nodejs/issues/7) and [Issue #10](https://github.com/onury/grunt-jasmine-nodejs/issues/10))
